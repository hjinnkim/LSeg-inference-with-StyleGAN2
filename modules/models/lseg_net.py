...
class LSeg(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer('logit_scale', (torch.ones([]) * np.log(1 / 0.07)).exp())
    ...
    def forward_logits(self, image_features, text_features, flip=False):
        # print('forward logits / flip: ', flip)
        logits_per_image = self.logit_scale * image_features.half() @ text_features.t()

        out = logits_per_image.float().view(self.imshape[0], self.imshape[2], self.imshape[3], -1).permute(0,3,1,2)
        out = self.scratch.output_conv(out)
        
        return out if not flip else flip_image(out)
    
    def set_imshape(self, x):
        with torch.no_grad():
            layer_1, layer_2, layer_3, layer_4 = forward_vit(self.pretrained, x)
            ...
            image_features = self.scratch.head1(path_1)
            self.imshape = image_features.shape
