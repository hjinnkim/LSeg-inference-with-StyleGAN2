...
def main(ctx: click.Context, network_pkl: str, dataset, save_path, flip, mask, no_seg, fp16):
    ...
    for idx, seed in enumerate(tqdm(seeds)):
        ...
        img_t = G(z, label, truncation_psi=truncation_psi, noise_mode='const')
        img = (img_t.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
